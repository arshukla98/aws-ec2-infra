...
const CROSSPLANE_HOST = "Crossplane_Host";
...
export async function crossplaneEC2Router(options: RouterOptions): Promise<express.Router> {
    const { logger } = options;
    const router = Router();
    router.get('/ec2', async (_, response) => {
            ...
            if (data && data.Reservations) {
                data.Reservations.forEach(reservation => {
                    reservation.Instances?.forEach(instance => {
                        ...
                        if (instanceName === CROSSPLANE_HOST) {
                            instances.push(`${instancePublicIP}`);
                        }
                    });
                });
            }
